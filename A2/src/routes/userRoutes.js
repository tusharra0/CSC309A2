...
// GET /users 
// Get all users with filters (requires manager or higher clearance)
router.get('/', authRequired, requireRole(['manager', 'superuser']), getAllUsers);

// POST /users 
// Create new user (requires cashier or higher clearance)
...
// PATCH /users/me 
// Update current logged-in user's information (requires authentication)
...
// Update current logged-in user's password (requires authentication)
router.patch('/me/password', authRequired, requireRole(['regular', 'cashier', 'manager', 'superuser']), updateMyPassword);

// GET /users/:id
// Get a specific user by ID (requires cashier or higher clearance)
...
// PATCH /users/:id
// Update a specific user by ID (requires manager or higher clearance)
